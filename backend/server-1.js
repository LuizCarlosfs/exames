...
try {
    ...
    console.error(`Erro ao inicializar o cliente GenAI. Verifique sua GOOGLE_API_KEY: ${e}`);
    process.exit(1);
}
...
async function callAgent(agentInstructions, messageText) {
    ...
    const result = await chat.sendMessage(`${agentInstructions}\n\n${messageText}`);
...
// --- AGENT FUNCTIONS (mantêm-se as mesmas, apenas para referência) ---

async function agenteBuscador(topic, age, gender, medicalSpecialty, additionalText, allFilesContent, todayDate) {
    const instruction = `
    ...
    const filesInfoText = allFilesContent.length ? allFilesContent.map((fileData, i) =>
...
app.post('/generate-post', upload.any(), async (req, res) => {
    ...
    if (!genAI) {
        return res.status(500).json({ error: "GenAI Client not initialized. Check API Key." });
    }
    ...
    try {
        const analiseMedicaInicial = await agenteBuscador(topic, age, gender, medicalSpecialty, additionalText, allFilesContent, todayDate);
...
app.listen(port, () => {
    if (!API_KEY) {
        console.warn("AVISO: A variável de ambiente GOOGLE_API_KEY não está configurada. Por favor, defina-a em seu arquivo .env ou diretamente no ambiente.");
    }
