...
import pdfParse from "pdf-parse" // <--- VOLTE PARA ESTA IMPORTAÇÃO
...
async function callAgent(agentInstructions, messageText) {
  ...
  const result = await chat.sendMessage(
    `<span class="math-inline">\{agentInstructions\}\\n\\n</span>{messageText}`
  )
  const response = await result.response
  return response.text()
}

// --- FUNÇÃO extractTextFromFile ATUALIZADA PARA PDF-PARSE ---
async function extractTextFromFile(file) {
  ...
  if (file.mimetype === "application/pdf") {
    try {
      const data = await pdfParse(file.buffer) // <--- USANDO PDF-PARSE COM BUFFER
      return data.text
    } catch (e) {
    ...
    return `Tipo de arquivo não suportado para extração de texto: '<span class="math-inline">\{file\.originalname\}' \(</span>{file.mimetype})`
  }
}
...
async function agenteBuscador(
  ...
  const filesInfoText = allFilesContent.length
    ? allFilesContent
        .map(
          (fileData, i) =>
            `--- INÍCIO DO CONTEÚDO DO ARQUIVO <span class="math-inline">\{i \+ 1\} \(</span>{
              fileData.filename
...
app.post("/generate-post", upload.any(), async (req, res) => {
  ...
  try {
    ...
    const planoDeRelatorio = await agentePlanejador(
      topic,
      analiseMedicaInicial
    )
    const rascunhoDeRelatorio = await agenteRedator(topic, planoDeRelatorio)
    const parecerFinalRevisado = await agenteRevisor(
      topic,
      rascunhoDeRelatorio
    )
